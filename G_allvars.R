## load("/mnt/phs/group/bdomingu/data_clean_bd.Rdata")
## robust.se <- function(model, cluster=df$family){
##     require(sandwich)
##     require(lmtest)
##     M <- length(unique(cluster))
##     N <- length(cluster)
##     K <- model$rank
##     dfc <- (M/(M - 1)) * ((N - 1)/(N - K))
##     uj <- apply(estfun(model), 2, function(x) tapply(x, cluster, sum));
##         rcse.cov <- dfc * sandwich(model, meat = crossprod(uj)/N)
##     rcse.se <- coeftest(model, rcse.cov)
##     return(list(rcse.cov, rcse.se)[[2]])
## }

## paste(paste("m_pc_bri_",1:10,sep=""),collapse="+")->covars
## #paste(paste("c_pc_bri_",1:10,sep=""),collapse="+")->covars2
## #paste(covars,"+",covars2,sep="")->covars


## c("health.pc","bmi","hlth","smk","cig","alc","caff_day","drg","vit","sleep_probs")->v1
## c("ses.pc", "edu","solo","job","mlv","benefits_received","sub_fin_diff","mdi")->v2
## c(v1,v2)->outcomes

## factor(df$edu)->df$edu

## ##################################################################################
## ##table 4
## std<-function(x) (x-mean(x,na.rm=TRUE))/sd(x,na.rm=TRUE)
## hold<-list()
## for (yv in c("eyfsp","ks1")) {
##     out<-list()
##     for (xv in outcomes) {
##         df[[xv]]->df$xv
##         paste(yv,"~m_pgs_ea+c_pgs_ea+xv+",covars,sep="")->f1
##         as.formula(f1)->f1
##         df[,c("motherid",all.vars(f1))]->tmp
##         tmp[rowSums(is.na(tmp))==0,]->tmp
##         std(tmp$m_pgs_ea)->tmp$m_pgs_ea
##         std(tmp$c_pgs_ea)->tmp$c_pgs_ea
##         std(tmp[[yv]])->tmp[[yv]]
##         #std(tmp$xv)->tmp$xv
##         lm(update.formula(f1,".~.-xv"),tmp)->m1
##         robust.se(m1,tmp$motherid)->m1
##         lm(f1,tmp)->m2
##         robust.se(m2,tmp$motherid)->m2
##         c(m1[2,1],m2[2,1],m2[2,2])->out[[xv]]
##     }
##     out->hold[[yv]]
## }

## makeci<-function(cil,cih) paste(format(round(cil,3),nsmall=3),format(round(cih,3),nsmall=3),sep=", ")
## f<-function(tab) {
##     do.call("rbind",tab)->tab
##     tab[,2]-1.96*tab[,3]->cil
##     tab[,2]+1.96*tab[,3]->cih
##     makeci(cil,cih)->ci
##     tab[,1]-tab[,2]->del
##     del/tab[,1] -> per
##     cbind(tab[,1],tab[,2],ci,per)->tab
##     tab
## }
## lapply(hold,f)->tab
## lapply(tab,write.csv)


